...
Renderer::Renderer()
{
	...
	// --- WINDOW ---
	m_window = SDL_CreateWindow("SDL...", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 800, 600, SDL_WINDOW_SHOWN);
...
void Renderer::LoadTextureText(const std::string &fontId, Text text) {
	SDL_Surface	*tmpSurf = TTF_RenderText_Blended(m_fontData[fontId], text.text.c_str(), SDL_Color{ text.myColor.r, text.myColor.g, text.myColor.b,text.myColor.a });
...
vector2f Renderer::GetTextureSize(const std::string &id) {
	int w; int h;
